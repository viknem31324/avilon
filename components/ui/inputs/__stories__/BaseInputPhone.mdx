...
InputPhone может использовать локали для форматирования номера телефона. Для этого необходимо указать массив поддерживаемых локалей `localesPhone` и указать текущую локаль в атрибуте `locale`(например, `'ru'`, `'by'`).
Элементы `localesPhone` состоят из следующих полей:

* `locale`(*string*\[required]): название локали(например, `'ru'`, `'by'`).
* `label`(*string*\[optional]): label для текущей локали.
* `mask`(*string*\[optional]): mask для текущей локали.
* `placeholder`(*string*\[optional]): placeholder для текущей локали.
